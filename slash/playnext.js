...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("playnext")
        .setDescription("Loads a song or playlist from YouTube or Spotify and adds it to the front of the queue")
        ...
        run: async({ client, interaction }) => {
            ...
            if (isValidUrl && (input.includes("youtube") || input.includes("youtu.be"))) { // handle YouTube links
                if (!input.includes("playlist")) {
                    ...
                    await queue.insertTrack(song, 0)
                    embed
                        .setDescription(`**[${song.title}](${song.url})** has been added to the front of the queue`)
                    ...
                    // loop through songs in playlist, inserting elements from last to first to the front of the queue
                    for (let i = result.tracks.length - 1; i >= 0; i--) {
                        await queue.insertTrack(result.tracks[i], 0)
                    }
                    embed
                        .setDescription(`**${result.tracks.length} songs from [${playlist.title}](${playlist.url})** have been added to the front of the queue`)
